            ...
            <el-form ref="form" :rules="addFormRules" :model="form" label-width="80px">
                ...
                <el-form-item
                    ...
                    <el-select v-model="form.$!{column.name}" clearable filterable
                               placeholder="请选择#if($column.comment.endsWith('ID'))$!{column.comment.replaceAll('ID','')}#elseif($column.comment.contains('('))$!{column.comment.substring(0,$column.comment.indexOf('('))}#else$!{column.comment}#end"
    ...
    export default {
        ...
        :
            ...
            :
                [{
                    required: true,
                    message: '请输入#if($column.comment.endsWith('
                    ID'))$!{column.comment.replaceAll('
                    ID','
                    ')}#elseif($column.comment.contains('(
                    '))$!{column.comment.substring(0,$column.comment.indexOf('(
                    '))}#else$!{column.comment}#end'
                }],//$!{column.comment}
                    #end
                #end
            }
